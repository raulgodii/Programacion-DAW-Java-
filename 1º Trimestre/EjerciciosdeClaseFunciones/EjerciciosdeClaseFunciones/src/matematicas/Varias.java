...
/** 
   ...
   */
    public static Boolean esPrimo(long n) {
        boolean noDivisible = true;
        if(n<1){
            noDivisible = false;
        }
        
        for (long i = 2; (i <= n/2) && noDivisible; i++) {
            if (n % i == 0) {
                noDivisible = false;
            }
        }
  ...
  public static long voltea(long x) {
    // en caso de número negativo, llamamos a la funcion negando el número.
    if (x < 0) {
      return -voltea(-x);
    }
  ...
  /**
   * AÃ±ade un dígito a un número por detrás (por la derecha).
   *
   * @param x número entero
   ...
   */
    public static long pegaPorDetras(long x, int d) {
        long resultado = Long.MIN_VALUE;
        if(digitos(d)==1)
            resultado = juntaNumeros(x, d);
      return resultado;
    }
  ...
  /**
   * AÃ±ade un dígito a un número por delante (por la izquierda).
   *
   * @param x número entero
